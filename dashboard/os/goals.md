# Objetivos — Q2 2026

## SIMULIA — Objetivo: 1.000€ MRR
- **Actual:** 59€ MRR (6%) — 9 usuarios activos
- **Fecha límite:** 30 junio 2026
- **Métricas clave:**
  - Usuarios activos: 23 → objetivo 60
  - Nuevos/mes: 4 → objetivo 12
  - Churn mensual: < 5%
- **Palancas:** SEO orgánico, contenido oposiciones, optimización conversión

## AGENCIA — Objetivo: validar modelo de cliente
- **Objetivo actual:** 5 clientes/mes (validación)
- **Fecha límite:** 30 junio 2026
- **Métricas clave:**
  - Clientes captados/mes: 0 → objetivo 5
  - Ingreso mensual Agencia: en construcción
- **Palancas:** contenido que genera confianza y leads, oferta clara, CTA en vídeos
- **Nota:** el foco NO son suscriptores sino conversión a cliente

## MOCCA — PAUSADO
- Sin objetivos activos hasta tener presupuesto
- Retomar cuando Simulia supere 800€ MRR
